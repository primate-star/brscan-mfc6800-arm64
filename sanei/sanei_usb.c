...
typedef struct
{
  ...
  SANE_Int interrupt_in_ep;  /* Interrupt endpoint for status polling */
...
sanei_usb_open (SANE_String_Const devname, SANE_Int * dn)
{
  ...
  if (devices[devcount].method == sanei_usb_method_libusb)
      ...
      if (result < 0)
	{
      ...
      interface = &dev->config[0].interface[devices[devcount].interface_nr].altsetting[0];
      ...
      for (num = 0; num < interface->bNumEndpoints; num++)
	{
	  ...
	  /* Handle interrupt endpoints for status polling */
	  if (transfer_type == USB_ENDPOINT_TYPE_INTERRUPT)
	    {
	      DBG (5, "sanei_usb_open: found interrupt-%s endpoint (address 0x%02x)\n",
		   direction ? "in" : "out", endpoint->bEndpointAddress);
	      if (direction)	/* in */
		{
		  if (!devices[devcount].interrupt_in_ep)
		    {
		      devices[devcount].interrupt_in_ep = endpoint->bEndpointAddress;
		      DBG (3, "sanei_usb_open: using interrupt-in endpoint 0x%02x for status\n",
...
#ifdef HAVE_LIBUSB
  /* Clear any stall conditions on endpoints after opening */
  if (devices[devcount].method == sanei_usb_method_libusb)
    {
      if (devices[devcount].bulk_in_ep)
	usb_clear_halt (devices[devcount].libusb_handle, devices[devcount].bulk_in_ep);
      if (devices[devcount].bulk_out_ep)
	usb_clear_halt (devices[devcount].libusb_handle, devices[devcount].bulk_out_ep);
      DBG (3, "sanei_usb_open: cleared endpoint halts\n");
    }
#endif
...
sanei_usb_read_bulk (SANE_Int dn, SANE_Byte * buffer, size_t * size)
{
  ...
  if (devices[dn].method == sanei_usb_method_scanner_driver)
	...
	{
	  DBG (2, "sanei_usb_read_bulk: reading %lu bytes ep=0x%02x timeout=%d\n",
	       (unsigned long)*size, devices[dn].bulk_in_ep, libusb_timeout);

	  /* Simple direct read - USB host controller handles packetization */
	  ...
	  if (read_size < 0)
	    {
	      DBG (2, "sanei_usb_read_bulk: usb_bulk_read error=%ld: %s\n",
		   (long)read_size, usb_strerror());
	    }
	  else
	    {
	      DBG (2, "sanei_usb_read_bulk: got %ld bytes\n", (long)read_size);
	    }
...
sanei_usb_write_bulk (SANE_Int dn, const SANE_Byte * buffer, size_t * size)
{
  ...
  if (devices[dn].method == sanei_usb_method_scanner_driver)
	...
	{
	  /* Chunked write for USB 1.1 compatibility - EP3 OUT max packet size is 16 bytes */
	  int chunk_size = 16;
	  int total_written = 0;
	  int remaining = (int) *size;

	  DBG (2, "sanei_usb_write_bulk: writing %lu bytes in %d-byte chunks to ep=0x%02x\n",
	       (unsigned long)*size, chunk_size, devices[dn].bulk_out_ep);

	  while (remaining > 0)
	    {
	      int to_write = (remaining > chunk_size) ? chunk_size : remaining;
	      int bytes = usb_bulk_write (devices[dn].libusb_handle,
					  devices[dn].bulk_out_ep,
					  (const char *) buffer + total_written,
					  to_write, libusb_timeout);

	      if (bytes < 0)
		{
		  DBG (1, "sanei_usb_write_bulk: chunk write error %d at offset %d\n",
		       bytes, total_written);
		  if (total_written == 0)
		    {
		      write_size = bytes;  /* Return error if nothing written */
		    }
		  else
		    {
		      write_size = total_written;  /* Return partial write */
		    }
		  break;
		}
	      ...
	      total_written += bytes;
	      remaining -= bytes;
	      DBG (2, "sanei_usb_write_bulk: wrote chunk of %d bytes, total %d/%lu\n",
		   bytes, total_written, (unsigned long)*size);

	      /* Small delay between chunks to let USB 1.1 device process */
	      if (remaining > 0)
		usleep(1000);
	    }
...
/* Read from interrupt endpoint - used for status polling on devices like
 * Brother MFC-6800 where EP5 (0x85) signals when scan data is ready */
SANE_Status
sanei_usb_read_int (SANE_Int dn, SANE_Byte * buffer, size_t * size)
{
  ...
  if (devices[dn].method == sanei_usb_method_scanner_driver)
    {
      DBG (1, "sanei_usb_read_int: interrupt read not supported for scanner driver\n");
      return SANE_STATUS_UNSUPPORTED;
    }
  else if (devices[dn].method == sanei_usb_method_libusb)
#ifdef HAVE_LIBUSB
    {
      if (devices[dn].interrupt_in_ep)
	{
	  DBG (2, "sanei_usb_read_int: reading %lu bytes from interrupt ep=0x%02x\n",
	       (unsigned long)*size, devices[dn].interrupt_in_ep);

	  /* Use shorter timeout for interrupt polling (500ms) */
	  read_size = usb_interrupt_read (devices[dn].libusb_handle,
					  devices[dn].interrupt_in_ep,
					  (char *) buffer,
					  (int) *size,
					  500);
	  ...
	  if (read_size < 0)
	    {
	      DBG (3, "sanei_usb_read_int: interrupt read returned %ld\n", (long)read_size);
	      *size = 0;
	      return SANE_STATUS_IO_ERROR;
	    }

	  DBG (2, "sanei_usb_read_int: got %ld bytes from interrupt endpoint\n",
	       (long)read_size);
	}
      else
	{
	  DBG (1, "sanei_usb_read_int: no interrupt-in endpoint available\n");
	  return SANE_STATUS_UNSUPPORTED;
	}
    }
#else /* not HAVE_LIBUSB */
  ...
  if (read_size == 0)
    {
      DBG (3, "sanei_usb_read_int: read returned no data\n");
      *size = 0;
      return SANE_STATUS_EOF;
    }
